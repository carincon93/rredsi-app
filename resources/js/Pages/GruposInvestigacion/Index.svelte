<script>
    ...
    function destroy() {
        if (isSuperAdmin || checkPermission(authUser, [3])) {
            Inertia.delete(route('instituciones-educativas.grupos-investigacion.destroy', [institucionEducativa.id, grupoInvestigacionId]), {
...
<AuthenticatedLayout>
    ...
    <DataTable routeParams={[institucionEducativa.id]}>
        ...
        <div slot="actions">
            {#if isSuperAdmin || checkPermission(authUser, [3])}
                <Button on:click={() => Inertia.visit(route('instituciones-educativas.grupos-investigacion.create', [institucionEducativa.id]))} variant="raised" class="bg-gray-500">Crear grupo de investigación</Button>
            {/if}
        </div>
                            ...
                            <div class="flex justify-center items-center">
                                <DataTableMenu class={gruposInvestigacion.data.length < 4 ? 'z-50' : ''}>
                                    {#if isSuperAdmin || checkPermission(authUser, [4])}
                                        <Item on:SMUI:action={() => Inertia.visit(route('instituciones-educativas.grupos-investigacion.show', [institucionEducativa.id, grupoInvestigacion.id]))}>
                                            <Text>Ver detalles</Text>
                                        </Item>
                                    {/if}
                                    {#if isSuperAdmin || checkPermission(authUser, [3])}
                                        <Item on:SMUI:action={() => Inertia.visit(route('instituciones-educativas.grupos-investigacion.edit', [institucionEducativa.id, grupoInvestigacion.id]))}>
                                            <Text>Editar</Text>
                                        </Item>
                                    {/if}
                                    {#if isSuperAdmin || checkPermission(authUser, [3])}
                                        <Item on:SMUI:action={() => ((grupoInvestigacionId = grupoInvestigacion.id), (dialogOpen = true))}>
                                            <Text>Eliminar</Text>
                                        </Item>
                                    {/if}
                                </DataTableMenu>
