<script>
    ...
    function destroy() {
        if (isSuperAdmin || checkPermission(authUser, [7])) {
            Inertia.delete(route('instituciones-educativas.semilleros-investigacion.destroy', [institucionEducativa.id, semilleroInvestigacionId]), {
...
<AuthenticatedLayout>
    ...
    <DataTable routeParams={[institucionEducativa.id]}>
        ...
        <div slot="actions">
            {#if isSuperAdmin || checkPermission(authUser, [7])}
                <Button on:click={() => Inertia.visit(route('instituciones-educativas.semilleros-investigacion.create', [institucionEducativa.id]))} variant="raised" class="bg-gray-500">Crear semillero de investigación</Button>
            {/if}
        </div>
                            ...
                            <div class="flex justify-center items-center">
                                <DataTableMenu class={semillerosInvestigacion.data.length < 4 ? 'z-50' : ''}>
                                    {#if isSuperAdmin || checkPermission(authUser, [7, 8])}
                                        <Item on:SMUI:action={() => Inertia.visit(route('instituciones-educativas.semilleros-investigacion.integrantes', [institucionEducativa.id, semilleroInvestigacion.id]))}>
                                            <Text>Integrantes</Text>
                                        </Item>
                                    {/if}
                                    {#if isSuperAdmin || checkPermission(authUser, [8])}
                                        <Item on:SMUI:action={() => Inertia.visit(route('instituciones-educativas.semilleros-investigacion.show', [institucionEducativa.id, semilleroInvestigacion.id]))}>
                                            <Text>Ver detalles</Text>
                                        </Item>
                                    {/if}
                                    {#if isSuperAdmin || checkPermission(authUser, [7])}
                                        <Item on:SMUI:action={() => Inertia.visit(route('instituciones-educativas.semilleros-investigacion.edit', [institucionEducativa.id, semilleroInvestigacion.id]))}>
                                            <Text>Editar</Text>
                                        </Item>
                                    {/if}
                                    {#if isSuperAdmin || checkPermission(authUser, [7])}
                                        <Item on:SMUI:action={() => ((semilleroInvestigacionId = semilleroInvestigacion.id), (dialogOpen = true))}>
                                            <Text>Eliminar</Text>
                                        </Item>
                                    {/if}
                                </DataTableMenu>
