<script>
    ...
    function destroy() {
        if (isSuperAdmin || checkPermission(authUser, [15])) {
            Inertia.delete(route('instituciones-educativas.eventos.destroy', [institucionEducativa.id, eventoId]), {
...
<AuthenticatedLayout>
    ...
    <DataTable routeParams={[institucionEducativa.id]}>
        <div slot="title">Eventos</div>

        <div slot="actions">
            {#if isSuperAdmin || checkPermission(authUser, [15])}
                <Button on:click={() => Inertia.visit(route('instituciones-educativas.eventos.create', [institucionEducativa.id]))} variant="raised" class="bg-gray-500">Crear evento</Button>
            {/if}
        </div>
                        ...
                        <div class="grid grid-cols-4">
                            ...
                            <div class="flex justify-center items-center">
                                <DataTableMenu class={eventos.data.length < 4 ? 'z-50' : ''}>
                                    {#if isSuperAdmin || checkPermission(authUser, [16])}
                                        <Item on:SMUI:action={() => Inertia.visit(route('instituciones-educativas.eventos.show', [institucionEducativa.id, evento.id]))}>
                                            <Text>Ver detalles</Text>
                                        </Item>
                                    {/if}
                                    {#if isSuperAdmin || checkPermission(authUser, [15])}
                                        <Item on:SMUI:action={() => Inertia.visit(route('instituciones-educativas.eventos.edit', [institucionEducativa.id, evento.id]))}>
                                            <Text>Editar</Text>
                                        </Item>
                                    {/if}
                                    {#if isSuperAdmin || checkPermission(authUser, [15])}
