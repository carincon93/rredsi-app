...
<div class="mb-20">
    <div class="grid grid-cols-2 min-h-[32.6rem] relative">
        ...
        <figure class="absolute right-[-14rem] top-[-4rem]" style="clip-path: polygon(21% 0%, 100% 0%, 100% 100%, 0% 100%);">
            <img src={'/storage/' + equipoFormacion.foto} alt="" class="w-[1000px] h-[667px] object-cover" />
        </figure>
    </div>
    ...
    <p class="mt-2">{equipoFormacion.facultad.nombre}</p>

    <p class="font-medium mt-4">Institución educativa</p>
    <div class="flex items-center mt-2">
        <figure class="mr-2">
            <img src="/storage/{equipoFormacion.facultad.institucion_educativa.logo}" class="h-8" alt="Logo {equipoFormacion.facultad.institucion_educativa.nombre}" />
        </figure>
        <p>{equipoFormacion.facultad.institucion_educativa.nombre}</p>
    </div>

    <hr class="block my-20" />

    <h1 class="text-2xl mb-5">Descripción del equipo</h1>
    <p class="whitespace-pre-line text-justify">{equipoFormacion.descripcion}</p>

    <hr class="block my-20" />

    <h1 class="text-2xl mb-5">Disciplinas asociadas</h1>
    <ul>
        {#each equipoFormacion.disciplinas_subarea_conocimiento as disciplina}
...
<Dialog bind:open={dialogOpen}>
    <div slot="title" class="flex items-center mb-8">
        <InfoMessage>Por favor póngase en contacto con el responsable del equipo</InfoMessage>
    </div>
    <div slot="content">
        ...
        <ul>
            <li class="mb-4">
                <div>
                    ...
                    {equipoFormacion.nombre_responsable}
                    <br />
                    <span>Correo electrónico: </span>
                    <a href="mailto:{equipoFormacion.correo_responsable}" class="underline">{equipoFormacion.correo_responsable}</a>
                </div>
            </li>
        </ul>
    </div>
